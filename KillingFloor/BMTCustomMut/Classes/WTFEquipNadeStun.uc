...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (ROLE == ROLE_Authority)
	{
		foreach CollidingActors (class 'Actor', A, DamageRadius, HitLocation)
		{
			...
			{
				K = KFMonster(A);
				p = Instigator.Spawn(Class'BMTCustomMut.WTFEquipNadeStunProj', instigator,, Location, K.Rotation);
    ...
    {
        Spawn(class'BMTCustomMut.WTFEquipNadeExplosionEmitter',,,HitLocation + HitNormal*20,rotator(HitNormal));
		Spawn(class'BMTCustomMut.WTFEquipNadeStunFlash',,,HitLocation + HitNormal*20,rotator(HitNormal));
    }
