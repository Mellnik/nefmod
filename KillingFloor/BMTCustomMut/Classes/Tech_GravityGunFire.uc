Class Tech_GravityGunFire extends Tech_HL2WeaponFire;
...
function DoTrace(Vector Start, Rotator Dir)
{
	local Vector X;
	local Actor Other;
	local Tech_HL_GravityGun G;
	local Tech_GravityGunPush P;
	local Tech_GravityGunKillWatch K;

	X = vector(Dir);
	G = Tech_HL_GravityGun(Weapon);
	if( G.bHasAttached )
	{
	...
	Other = Tech_HL_GravityGun(Weapon).FindGrabActor(Start,Dir,bOrganicMode);
	...
	if( Other!=None )
	{
		...
		if( Pawn(Other)!=None )
		{
			if( Pawn(Other).Default.Health<500 || G.bIsOrganic )
			{
				...
				if( G.bIsOrganic )
				{
					K = Weapon.Spawn(Class'Tech_GravityGunKillWatch',,,Other.Location);
					K.AttachTo(Pawn(Other));
				}
				else Other.TakeDamage(25,Instigator,Other.Location,vect(0,0,0),Class'DamTypeTech_GravGunKill');
			}
		}
		else
		{
			...
			{
				P = Spawn(Class'Tech_GravityGunPush',,,Other.Location);
