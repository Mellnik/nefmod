class Tech_DoomSentry extends Pawn
    Config(NetskyDoomSentry);
...
var() config int HitDamage,DoomSentryHealth;
...
var Emitter mTracer;
var Tech_DoomSentryBotWeaponFlash WeaponFlash;
var transient float NextVoiceTimer;
var Tech_DoomSentryBot WeaponOwner;
...
replication
{
    ...
    reliable if( Role==ROLE_Authority )
        RepAnimationAction,RepHitLocation,DoomSentryHealth;
}

final function SetOwningPlayer( Pawn Other, Tech_DoomSentryBot W )
{
    OwnerPawn = Other;
...
simulated function PostRender2D(Canvas C, float ScreenLocX, float ScreenLocY)
{
	...
	if( Health >= DoomSentryHealth )			// Lasvegaspunk
	{
		C.SetDrawColor(0,0,150,255); 	// Dark Blue
	}
	else if ( Health < DoomSentryHealth && Health >= (DoomSentryHealth * 0.85) )
	{
		C.SetDrawColor(0,225,0,255);	// Green
	}
	else if ( Health < DoomSentryHealth * 0.85 && Health >= (DoomSentryHealth * 0.65) )
	{
		C.SetDrawColor(255,255,51,255);		// Yellow
	}
	else if ( Health < DoomSentryHealth * 0.65 && Health >= (DoomSentryHealth * 0.35) )
	{
	...
	//S = "Health:"@Max(1,float(Health)/float(DoomSentryHealth)*100.f)@"%";
	S = "Health: "@Health@"/"@DoomSentryHealth;
	C.TextSize(S,XL,YL);
...
event PostBeginPlay()
{
    ...
    Health = DoomSentryHealth;
    HealthMax = DoomSentryHealth;
}
...
simulated function SetAnimationNum( byte Num )
{
    ...
    case 2:
        ...
        {
            WeaponFlash = Spawn(Class'Tech_DoomSentryBotWeaponFlash',Self);
...
function Timer()
{
    ...
    foreach TraceActors(Class'Actor',Res,HL,HN,Location+X*8000.f,Location)
    {
        if( Res!=Self && (Res==Level || Res.bBlockActors || Res.bProjTarget || Res.bWorldGeometry) && KFPawn(Res)==None
             && KFBulletWhipAttachment(Res)==None && Tech_DoomSentry(Res)==None )
        {
            A = Res;
            break;
        }
    ...
    if( A!=None && KFPawn(A)==None )
        A.TakeDamage(HitDamage,OwnerPawn,HL,X*1000.f,Class'DamTypeTech_DoomSentryFire');
    else if( A==None )
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
    ...
    if( WeaponOwner!=None )
    {
        if( OwnerPawn!=None && PlayerController(OwnerPawn.Controller)!=None )
            PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Tech_DoomSentryBotMessage',2);
...
simulated function HurtRadius( float DamageAmount, float DamageRadius, class<DamageType> DamageType, float Momentum, vector HitLocation )
{
    ...
    foreach CollidingActors (class 'Actor', Victims, DamageRadius, HitLocation)
    {
        ...
        if( (Victims != self) && (Victims.Role == ROLE_Authority) && !Victims.IsA('FluidSurfaceInfo')
         && ExtendedZCollision(Victims)==None && KFPawn(Victims)==None && Tech_DoomSentry(Victims)==None )
        {
...
defaultproperties
{
     ...
     Health=1200
     ControllerClass=Class'BMTCustomMut.Tech_DoomSentryBotController'
     HitDamageType=Class'BMTCustomMut.DamTypeTech_DoomSentryFire'
     bPhysicsAnimUpdate=True
