class Tech_USCMSentryGun extends SVehicle
    Placeable
    Config(NetskyUSCMSentry)
    CacheExempt;
...
var transient Tech_USCMSentryMuzzleFlash Flashes[4];
...
var Pawn OwnerPawn;
var Tech_USCMSentry WeaponOwner;
...
final function SetOwningPlayer( Pawn Other, Tech_USCMSentry Wep )
{
    OwnerPawn = Other;
...
simulated function PostRender2D(Canvas C, float ScreenLocX, float ScreenLocY)
{
	...
	if( Health >= TurretHealth )			// Lasvegaspunk
	{
		C.SetDrawColor(0,0,150,255); 	// Dark Blue
	}
	else if ( Health < TurretHealth && Health >= (TurretHealth * 0.85) )
	{
		C.SetDrawColor(0,225,0,255);	// Green
	}
	else if ( Health < TurretHealth * 0.85 && Health >= (TurretHealth * 0.65) )
	{
		C.SetDrawColor(255,255,51,255);		// Yellow
	}
	else if ( Health < TurretHealth * 0.65 && Health >= (TurretHealth * 0.35) )
	{
    ...
    if( Level.NetMode!=NM_Client && Physics==PHYS_Karma )
    {
        ...
        if( KParams!=None && KParams.bContactingLevel && NextFlipCheckTime<Level.TimeSeconds )
        {
            ...
            if( bFlip!=bIsCurrentlyFlipped )
            {
                ...
                Tech_USCMSentryAI(Controller).NotifyGotFlipped(bFlip);
            }
        }
    }
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, class<DamageType> damageType, optional int HitIndex )
{
    ...
    if( Health<=0 )
    {
        ...
        Tech_USCMSentryAI(Controller).NotifyGotFlipped(true);
    }
}
...
simulated function Timer()
{
    ...
    if( (Level.TimeSeconds-LastRenderTime)<4 )
    {
        ...
        {
            Flashes[FXBarrelNumber] = Spawn(Class'Tech_USCMSentryMuzzleFlash');
            AttachToBone(Flashes[FXBarrelNumber],BarrelBones[FXBarrelNumber]);
            Flashes[FXBarrelNumber+1] = Spawn(Class'Tech_USCMSentryMuzzleFlash');
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
    ...
    if( WeaponOwner!=None )
    {
        if( OwnerPawn!=None && PlayerController(OwnerPawn.Controller)!=None )
            PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Tech_USCMSentryMessageTurret',2);
...
defaultproperties
{
     ...
     HitDamages=37
     TurretHealth=1200
     HitDgeType=Class'BMTCustomMut.DamTypeTech_USCMSentry'
     ...
     MenuName="USCM Turret"
     ControllerClass=Class'BMTCustomMut.Tech_USCMSentryAI'
     bStasis=False
     ...
     End Object
     KParams=KarmaParamsRBFull'BMTCustomMut.Tech_USCMSentryGun.KarmaParamsRBFull0'
