...
simulated function PostBeginPlay()
{
	...
	if ( Level.NetMode != NM_DedicatedServer)
    {
		l=location;
		l.z-=10;
        aem = Spawn(class'SPAura',self,,l,rot(0,0,1));
        aem.SetBase(self);
		aem.SetRotation(rot(0,0,1));
    }
}
...
function ServerBuyWeapon( Class<Weapon> WClass, float ItemWeight )
{
	...
	if( WClass==class'DualDeagle' || WClass==class'Dual44Magnum' || WClass==class'DualMK23Pistol' || WClass==class'DualFlareRevolverPickup' || WClass.Default.DemoReplacement!=None )
	{
	...
	else if( WClass==class'Single' || WClass==class'Deagle' || WClass==class'Magnum44Pistol' || WClass==class'MK23Pistol' || WClass==class'FlareRevolverPickup' )
	{
...
function VeterancyChanged()
{
	...
	super(KFHumanPawn).VeterancyChanged();
	
	for( I=Inventory; I!=None; I=I.Inventory )
	{
		if(InvPS(I)!=None && KFPlayerReplicationInfo(Controller.PlayerReplicationInfo).ClientVeteranSkill.default.PerkIndex==51 )
		{
			b=true;
			break;
		}
		else if( InvPS(I)!=None && KFPlayerReplicationInfo(Controller.PlayerReplicationInfo).ClientVeteranSkill.default.PerkIndex!=51 )
		{
			I.Destroy();
			return;
		}
	...
	if(KFPlayerReplicationInfo(Controller.PlayerReplicationInfo).ClientVeteranSkill.default.PerkIndex==51 && !b)
	{
		ips = Self.Spawn(class'InvPS', Self,,,rot(0,0,0));
		ips.GiveTo(Self);
		ips.InitTimer();		
	}
}
