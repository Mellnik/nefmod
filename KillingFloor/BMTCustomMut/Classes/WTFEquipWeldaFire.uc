...
/*simulated Function Timer()
{
	...
	local int MeleeDamage;
	local KFPlayerReplicationInfo KFPRI;

	If( !KFWeapon(Weapon).bNoHit )
	{
		MeleeDamage = MeleeDamage + Rand(MaxAdditionalDamage);
		
		KFPRI = KFPlayerReplicationInfo(Instigator.PlayerReplicationInfo);
		if ( KFPRI != none && KFPRI.ClientVeteranSkill != none )
		{
			MeleeDamage = float(MeleeDamage) * KFPRI.ClientVeteranSkill.Static.GetWeldSpeedModifier(KFPRI);
		}
		...
		if( LHitActor!=none && Level.NetMode!=NM_Client )
		{
			...
			if ( LHitActor.IsA('KFMonster') )
			{
				if ( LHitActor.IsA('WTFZombiesMetalClot') )
					MeleeDamage*=2.5;
				if (KFPRI != none && KFPRI.ClientVeteranSkill == Class'BMTCustomMut.WTFPerksFirebug')
					hitDamageClass = Class'WTFEquipDamTypeWelda';
			}

			if (ExtendedZCollision(HitActor) != none)
				HitActor.Base.TakeDamage(MeleeDamage, Instigator, HitLocation , vector(PointRot),hitDamageClass);
			else
				HitActor.TakeDamage(MeleeDamage, Instigator, HitLocation , vector(PointRot),hitDamageClass);
...
defaultproperties
{
     ...
     AmmoClass=Class'BMTCustomMut.WTFEquipWeldaAmmo'
}
