class Tech_DoomSentryBot extends KFWeapon;
...
var Tech_DoomSentry CurrentDoomSentry;
...
simulated function Destroyed()
{
	...
	{
		// Hack to switch to another weapon on the client when we throw the DoomSentry bot is destroyed.
...
event ServerStartFire(byte Mode)
{
	...
	if( CurrentDoomSentry==None )
	{
		...
		{
			CurrentDoomSentry = Spawn(Class'Tech_DoomSentry',,,Spot,R);
			if( CurrentDoomSentry!=None )
			{
				if( PlayerController(Instigator.Controller)!=None )
					PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Tech_DoomSentryBotMessage',1);
		...
		if( PlayerController(Instigator.Controller)!=None )
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Tech_DoomSentryBotMessage',0);
	}
}
...
defaultproperties
{
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'BMTCustomMut.Tech_DoomSentryBotPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'BMTCustomMut.Tech_DoomSentryBotAttachment'
