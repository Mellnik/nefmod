...
class MedSentryGun extends KFWeapon;

var MedSentry CurrentMedSentry;
...
simulated function Destroyed ()
{
  ...
  if ( (Level.NetMode != 3) && (CurrentMedSentry != None) )
  {
    CurrentMedSentry.WeaponOwner = None;
    CurrentMedSentry.KilledBy(None);
    CurrentMedSentry = None;
  }
  Super.Destroyed();
}
...
event ServerStartFire (byte Mode)
{
  ...
  if ( CurrentMedSentry == None )
  {
    R.Yaw = Instigator.Rotation.Yaw;
    Spot = vector(R) * (Instigator.CollisionRadius + 70.0) + Instigator.Location;
    if ( FastTrace(Spot,Instigator.Location) )
    {
      CurrentMedSentry = Spawn(Class'MedSentry',,,Spot,R);
      if ( CurrentMedSentry != None )
      {
        if ( PlayerController(Instigator.Controller) != None )
        {
          PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'MedSentryMessage',1);
        }
        CurrentMedSentry.SetOwningPlayer(Instigator,self);
    ...
    if ( PlayerController(Instigator.Controller) != None )
    {
      PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'MedSentryMessage',0);
    }
  }
}
...
{
     MagCapacity=1
     HudImage=Texture'Sentrybot_T.Sentry.MedI_Sentry'
     SelectedHudImage=Texture'Sentrybot_T.Sentry.MedI_Sentry'
     Weight=1.000000
     bModeZeroCanDryFire=True
     TraderInfoTexture=Texture'Sentrybot_T.Sentry.MedI_Sentry'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'BMTCustomMut.MedSentryGunPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'BMTCustomMut.MedSentryGunAttachment'
