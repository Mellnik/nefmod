...
// PTurret.
//=============================================================================
class RPTurret extends KFWeapon
	Config(RocketPortalTurret);

var RPortalTurret CurrentSentry;
...
event ServerStartFire(byte Mode)
{
	...
	if( CurrentSentry==None )
	{
		...
		if( FastTrace(Spot,Instigator.Location) )
		{
			...
			else */CurrentSentry = Spawn(Class'RPortalTurret',,,Spot,R);
			if( CurrentSentry!=None )
			{
				if( PlayerController(Instigator.Controller)!=None )
					PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'RTurretMessage',1);
		...
		if( PlayerController(Instigator.Controller)!=None )
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'RTurretMessage',0);
	}
}
...
{
     MagCapacity=1
	 HudImageRef="RocketPortalTurret.I_Turret"
	 SelectedHudImageRef="RocketPortalTurret.I_Turret"
     HudImage=Texture'RocketPortalTurret.I_Turret'
     SelectedHudImage=Texture'RocketPortalTurret.I_Turret'
     Weight=1.000000
     bModeZeroCanDryFire=True
     TraderInfoTexture=Texture'RocketPortalTurret.I_Turret'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'RocketPortalTurret.RPTurretPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'RPTurretAttachment'
