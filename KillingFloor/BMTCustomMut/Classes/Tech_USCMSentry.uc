class Tech_USCMSentry extends KFWeapon
    Config(Netsky);

var Tech_USCMSentryGun CurrentSentry;
...
var class<Tech_USCMSentryGun> SentryClass;
...
simulated function Destroyed()
{
    ...
    {
        // Hack to switch to another weapon on the client when we throw the USCMSentryGun bot is destroyed.
    ...
    if( CurrentSentry==None )
    {
        ...
        if( FastTrace(Spot,Instigator.Location) )
        {
			if( bStationaryTurret )
				CurrentSentry = Spawn(Class'Tech_USCMSentryStationary',,,Spot,R);
			else
				CurrentSentry = Spawn(Class'Tech_USCMSentryGun',,,Spot,R);
			if( CurrentSentry!=None )
            {
                if( PlayerController(Instigator.Controller)!=None )
                    PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Tech_USCMSentryMessageTurret',1);
        ...
        if( PlayerController(Instigator.Controller)!=None )
            PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Tech_USCMSentryMessageTurret',0);
    }
}
...
//Disabled Gturret cant find the file LVP
...
defaultproperties
{
     SentryClass=Class'BMTCustomMut.Tech_USCMSentryGun'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'BMTCustomMut.Tech_USCMSentryPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'BMTCustomMut.Tech_USCMSentryAttachment'
