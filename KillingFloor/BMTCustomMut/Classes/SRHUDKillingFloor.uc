...
simulated function DrawWeaponName(Canvas C)
{
	...
	//weapon name block
	if(AK47AssaultRifle(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.Ak47Level$")";
	else if(WTFKnife(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.KnifeLevel$")";
	else if(P57LLI(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.P57Level$")";
	else if(KrissMMedicGunA(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.KrissLevel$")";
	else if(MP7MMedicGunExt(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.WTFMP7Level$")";
	else if(WTFEquipKatana(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.WTFKatanaLevel$")";
	else if(WTFFlamer(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.WTFFlamerLevel$")";
	else if(SA80(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.SA80Level$")";
	else if(B94(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.B94Level$")";
	else if(DeagleExt(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.DeagleLevel$")";
	else if(DualDeagleExt(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.DualDeagleLevel$")";
	else if(HK417(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.HK417Level$")";
	else if(Bullpup(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.BullpupLevel$")";
	else if(Bizon(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.BizonLevel$")";
	else if(LilithKiss(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.LilithKissLevel$")";
	else if(WColt(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.WColtLevel$")";
	else if(M41AAssaultRifle(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.M41ALevel$")";
	else if(M249(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.M249Level$")";
	else if(litesaber(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.litesaberLevel$")";
	else if(Tech_ShockRifle(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.Tech_ShockRifleLevel$")";
	else if(Stinger(PawnOwner.Weapon)!=None)
		CurWeaponName$=" (Lv "$ClientRep.StingerLevel$")";
	
	C.Font  = GetFontSizeIndex(C, -1);
//	C.SetDrawColor(255, 50, 50, KFHUDAlpha); //Red boring normal text color
	C.SetDrawColor(0, 255, 0, KFHUDAlpha); //Green new color
	C.Strlen(CurWeaponName, XL, YL);
	
	//log("Draw name"@PawnOwner.Weapon.Class.Name);
	WProg=Class<SRVeterancyTypes>(KFPlayerReplicationInfo(PawnOwnerPRI).ClientVeteranSkill).static.GetWeaponProgress(ClientRep, PawnOwner.Weapon.Class.Name);

	// Diet Hud needs to move the weapon name a little bit or it looks weird
	if ( !bLightHud )
	{
//		C.SetPos((C.ClipX * 0.983) - XL, C.ClipY * 0.90); //Origonal bottom right area
		C.SetPos((C.ClipX * 0.25) - XL, C.ClipY * 0.02); //Top left
		C.DrawText(CurWeaponName);
		
		C.Strlen(WProg, XL, YL);
//		C.SetPos((C.ClipX * 0.983) - XL, C.ClipY * 0.80); //origonal bottom right area	
		C.SetPos((C.ClipX * 0.15) - XL, C.ClipY * 0.05); //Top left	
		C.DrawText(WProg);
	}
	else
	{
