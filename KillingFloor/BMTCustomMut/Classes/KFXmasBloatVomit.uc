...
class KFXmasBloatVomit extends BioGlob;

var bool TA;//switch for shooter use..
var actor BabyTarget;
...
function float DifficultyDamageModifer()
{
    ...
    if ( Level.Game.GameDifficulty >= 7.0 ) // Suicidal
    {
...
state OnGround
{
    ...
    {
        if ( Other != none && !Other.isA('ZombieBabyFleshPound') )
            BlowUp(Location);
    }
...
{
    local int g;
    local KFXmasBloatVomit NewGlob;
    local Vector VNorm;
...
auto state Flying
{
    simulated function Landed( Vector HitNormal )
    {
        ...
        local ZombieBabyFleshPound VOM;
        ...
        if(Ta)
        {
            VOM=spawn(class'ZombieBabyFleshPound',,,Self.location, rotator(-HitNormal));
            VOM.Controller.Target=BabyTarget;
        }
        TA=False;
        bCollideWorld = false;
        SetCollisionSize(GoopVolume*10.0, GoopVolume*10.0);
    ...
    {
        Landed(HitNormal);
        if ( !Wall.bStatic && !Wall.bWorldGeometry && !wall.isA('ZombieBabyFleshPound'))
        {
            bOnMover = true;
            SetBase(Wall);
            if (Base == None)
    ...
    simulated function ProcessTouch(Actor Other, Vector HitLocation)
    {
        if( ExtendedZCollision(Other)!=None )
            Return;
        if (!Other.isA('ZombieBabyFleshPound')&&(Other != Instigator && (Other.IsA('Pawn') || Other.IsA('DestroyableObjective') || Other.bProjTarget)))
