Class Tech_GravityGunKillWatch extends Projectile
	transient;
...
function Tick( float Delta )
{
	...
	{
		SpecimenTarget.TakeDamage(400,instigator,SpecimenTarget.Location,vect(0,0,0),Class'DamTypeTech_BGravGunKill');
		Destroy();
		return;
	}
...
function Touch( Actor Other )
{
	...
	if( Monster(Other)!=None && Other!=SpecimenTarget && Pawn(Other).Health>0 )
	{
		...
		{
			Other.TakeDamage(Dam,instigator,SpecimenTarget.Location,SpecimenTarget.Velocity*SpecimenTarget.Mass,Class'DamTypeTech_BGravGunKill');
			Dam = Dam<<1; // Make sure the thrown specimen dies now.
		}
		else
		{
			Dam = Pawn(Other).Health;
			Other.TakeDamage(99999,instigator,SpecimenTarget.Location,SpecimenTarget.Velocity*SpecimenTarget.Mass,Class'DamTypeTech_BGravGunKill');
		}
		SpecimenTarget.TakeDamage((Dam>>2),instigator,SpecimenTarget.Location,vect(0,0,0),Class'DamTypeTech_BGravGunKill');
	}
}
