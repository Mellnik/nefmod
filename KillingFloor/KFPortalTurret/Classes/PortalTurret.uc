// Written by Marco
Class PortalTurret extends SVehicle // Gay hack, but has to be done.
	Placeable
	Config(KFPortalTurret)
	CacheExempt;

#exec obj load file="PortalTurret.ukx" package="KFPortalTurret"
...
final function SetOwningPlayer( Pawn Other, PTurret Wep )
{
	OwnerPawn = Other;
...
{
	// Make noise
	PlaySound(Sound'TShootClose',SLOT_Pain,2.f,,800.f);
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	if( WeaponOwner!=None )
	{
		if( OwnerPawn!=None && PlayerController(OwnerPawn.Controller)!=None )
			PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'TurretMessage',2);
...
{
	if( PortalTurret(other)!=None && VSizeSquared(Velocity)>VSizeSquared(other.Velocity) )
		Speak(1);
}
...
defaultproperties
{
	...
	bBlockKarma=True
	ControllerClass=Class'TurretAI'
	AlarmNoiseSnd=Sound'Talert'
	...
	bNetNotify=False
	HitDamages=5
	HitDgeType=Class'DamTypeTurret'
	Skins(0)=Combiner'JTurretA'
	Team=250
	...
	BarrelBones(0)="RT_Gun1"
	BarrelBones(1)="LFT_Gun1"
	BarrelBones(2)="RT_Gun2"
	BarrelBones(3)="LFT_Gun2"

	Voices(0)=Sound'GTurretActivate'
	Voices(1)=Sound'GTurretCollide'
	Voices(2)=Sound'GTurretDisabled'
	Voices(3)=Sound'GTurretFF'
	Voices(4)=Sound'GTurretPickup'
	Voices(5)=Sound'GTurretSearching'
	Voices(6)=Sound'GTurretSleep'
	Voices(7)=Sound'GTurretTipped'

	RotationRate=(Yaw=25000,Pitch=25000)

	LaserOffset=(X=12,Z=11.8)
	TurretHealth=400
	Health=400
	bStasis=false
	...
	Begin Object Class=KarmaParamsRBFull Name=KarmaParamsRBFull0
		...
		KImpactThreshold=45
		KInertiaTensor(0)=0.1
		KInertiaTensor(1)=0
		KInertiaTensor(2)=0
		KInertiaTensor(3)=0.1
		KInertiaTensor(4)=0
		KInertiaTensor(5)=0.1
		KCOMOffset=(Y=-0.02)
	End Object
