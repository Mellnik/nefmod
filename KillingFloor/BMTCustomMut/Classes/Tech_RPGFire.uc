Class Tech_RPGFire extends Tech_HL2WeaponFire;
...
function Projectile SpawnProjectile(Vector Start, Rotator Dir)
{
	...
	if( p!=None )
	{
		Tech_HL_RPG(Weapon).bTrackingProjectile = true;
		if( Tech_RPGProjectile(p)!=None )
		{
			Tech_RPGProjectile(p).TrackingTarget = Tech_HL_RPG(Weapon).TargetDot;
			Tech_RPGProjectile(p).MyWeapon = Tech_HL_RPG(Weapon);
		}
	}
    return p;	
}
simulated function ModeTick( float dt )
{
	if( bTrackingProj && FireAnimEnd<Level.TimeSeconds )
	{
		...
		if( Tech_HL_RPG(Weapon).bTrackingProjectile )
...
defaultproperties
{
     ...
     NoAmmoSound=None
     FireRate=1.200000
     AmmoClass=Class'Tech_HL_RPGAmmo'
     ShakeRotMag=(X=3.000000,Y=4.000000,Z=2.000000)
     ShakeRotRate=(X=10000.000000,Y=10000.000000,Z=10000.000000)
     ShakeOffsetMag=(X=3.000000,Y=3.000000,Z=3.000000)
     ProjectileClass=Class'Tech_RPGProjectile'
     aimerror=12.000000
}
