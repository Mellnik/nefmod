Class Tech_BugBaitProjectile extends Tech_HL2Projectile;
...
simulated function HitWall(vector HitNormal, actor Wall)
{
	if( Level.NetMode!=NM_DedicatedServer )
	{
		...
		Spawn(Class'Tech_FX_BugBaitHitFX');
	}
	if( Level.NetMode!=NM_Client )
	{
		if( Pawn(Wall)!=None && Pawn(Wall).Health>0 )
			Class'Tech_ZEDAttractInv'.Static.SetTargeting(Level,Location,true,Pawn(Wall));
		else Class'Tech_ZEDAttractInv'.Static.SetTargeting(Level,Location,true,,Location);
	}
	Destroy();
}
