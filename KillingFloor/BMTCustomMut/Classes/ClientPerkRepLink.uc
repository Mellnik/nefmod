...
// weapon levels LVP
var byte Ak47Level, KnifeLevel, P57Level, KrissLevel, WTFMP7Level, WTFKatanaLevel, WTFFlamerLevel, SA80Level, B94Level, DeagleLevel, DualDeagleLevel, HK417Level, BullpupLevel, BizonLevel, LilithKissLevel, WColtLevel, M41ALevel, M249Level, litesaberLevel, Tech_ShockRifleLevel, StingerLevel;
...
replication
{
	reliable if( Role==ROLE_Authority && bNetOwner )
		...
		//replication for weapon levels LVP
		Ak47Level, KnifeLevel, P57Level, KrissLevel, WTFMP7Level, WTFKatanaLevel, WTFFlamerLevel, SA80Level,
		B94Level, DeagleLevel, DualDeagleLevel, HK417Level, BullpupLevel, BizonLevel, LilithKissLevel, WColtLevel,
		M41ALevel, M249Level, litesaberLevel, Tech_ShockRifleLevel, StingerLevel;
...
simulated final function string GetWeaponProgress( class<SRCustomProgress> C )
{
	local SRCustomProgress S;
	
	for( S=CustomLink; S!=None; S=S.NextLink )
	{
		log("Check link"@S.Class.Name@C.Name);
		if( S.Class.Name==C.Name )
		{
			log("S"@S);
			return WDamProgress(S).GetWProgress();
		}
	}
	return "";
}
