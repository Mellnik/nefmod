Class SlotsMachine extends HudOverlay;

#exec Texture Import File=Textures\Failure.pcx Name=I_Fail Group="Icons" Mips=Off MASKED=1
#exec AUDIO IMPORT FILE="Sounds\alarm2.wav" NAME="FailAlarm" GROUP="FX"

var HUD InitHUD;
var class<SlotCard> Cards[3],ForcedWin,DoubleWin;
...
var array< class<SlotCard> > CardsList;
...
simulated function ClientGiveCard( byte i, class<SlotCard> Card )
{
	local int j;
...
simulated function ClientSetCard( int i, class<SlotCard> Card )
{
...
final function class<SlotCard> PickRandomCard()
{
	local float Desire;
	local int i;
...
final function int GetCardIndex( class<SlotCard> C )
{
	local int i;
