class Tech_MachineDualiesAttachment extends KFWeaponAttachment;
...
var Tech_MachineDualiesAttachment brother;
...
{
	local vector Sc;
	local Tech_MachineDualiesAttachment DA;
	...
	if( Mesh==BrotherMesh )
	{
		...
		if( brother==None )
		{
			ForEach DynamicActors(Class'Tech_MachineDualiesAttachment',DA)
			{
	...
	if( !bBeamEnabled )
	{
		if (TacShine == none )
		{
			TacShine = Spawn(Class'Tech_MachineDualies'.Default.TacShineClass,Owner,,,);
