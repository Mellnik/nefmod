Class Tech_HL_GravityGun extends Tech_HL2WeaponNA;

var() color EnergyColor;
var() float GrabRange;
var() Sound HoldLoopSound;
var Tech_GravityGunGrab ProjGrabInfo;
var Actor GrabbedActor;
var float DragInTime;
var vector GrabStartPos;
var transient vector CarryDest;
var transient Tech_FX_GravityGunDot EnergyDots[3];
var transient Tech_FX_GravityGunBolts Bolts[2];
...
{
	local Tech_GravityGunKillWatch K;
	...
	else if( bCarryingPawn && GrabbedActor!=None && Pawn(GrabbedActor).Health>0 )
	{
		...
		if( bIsOrganic )
		{
			K = Spawn(Class'Tech_GravityGunKillWatch',,,GrabbedActor.Location);
	...
	foreach CollidingActors(Class'Actor',Other,GrabRange,Start+X*GrabRange*0.5)
	{
		 ...
		 && VSize(Other.Location-Start)<GrabRange )
		{
			...
			if( (Best==None || D>BestD) && Instigator.FastTrace(Other.Location,Start) )
			{
				...
				foreach DynamicActors(Class'Tech_HL_GravityGun',GG)
...
defaultproperties
{
     ...
     FireModeClass(0)=Class'BMTCustomMut.Tech_GravityGunFire'
     FireModeClass(1)=Class'BMTCustomMut.Tech_GravityGunAltFire'
     Description="A Zero Point Energy Manipulator."
     Priority=50
     InventoryGroup=4
     GroupOffset=5
     PickupClass=Class'BMTCustomMut.Tech_HL_GravityGunPickup'
     PlayerViewPivot=(Yaw=-16384)
     AttachmentClass=Class'BMTCustomMut.Tech_GravityGunAttachment'
