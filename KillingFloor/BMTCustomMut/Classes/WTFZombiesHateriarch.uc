...
{
	local PipeBomb2Proj Mine;
	local float D,MinSafeD;
	
	Mine = PipeBomb2Proj(A);
	if (Mine == None)
	{
...
simulated function Tick(float DeltaTime)
{
	...
	if (Level.TimeSeconds > ForceCamTime)
	{
		FollowCam();
		ForceCamTime = Level.TimeSeconds + 0.5;
	}
}
*/
...
state FireChaingun
{
	...
	function AnimEnd( int Channel )
	{
		...
		{
			//is this necessary? ...|| !Controller.Target.IsA('PipeBomb2Proj')
...
state Charging
{
	...
	function Tick( float Delta )
	{
        ...
        else
        {
            ...
            GroundSpeed = OriginalGroundSpeed * 1.875; //match original top charging speed; modded to account for new base groundspeed
        }
...
defaultproperties
{
     LastEyeLaserTimeMaxRandomCooldown=5
     LastSneakedTimeMaxRandomCooldown=15
     LastMissileTimeMaxRandomCooldown=25
     LastChainGunTimeMaxRandomCooldown=10
     LastChargeTimeMaxRandomCooldown=15
     LastKnockDownTimeMaxRandomCooldown=3
     LastEyeLaserTimeMinCooldown=5
     LastSneakedTimeMinCooldown=15
     LastMissileTimeMinCooldown=5
     LastChainGunTimeMinCooldown=10
     LastChargeTimeMinCooldown=15
     LastKnockDownTimeMinCooldown=7
     SneakDuration=10.000000
     MissileDamageRadius=500.000000
     MeleeDamage=100
     HeadHealth=10000.000000
     ...
     ControllerClass=Class'BMTCustomMut.WTFZombiesHateriarchController'
}
