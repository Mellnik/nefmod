...
function PlayDyingSound()
{
	if( Level.NetMode!=NM_Client )
	{
    	if ( bGibbed )
    	{
    		PlaySound(Sound'Inf_Weapons.satchel.satchel_explode011', SLOT_Pain,2.0,true,525);
    		return;
    	}
...
function SpawnTwoShots()
{
	...
	FireStart = Location+(vect(0,0,8) >> Rotation)*DrawScale;
	if ( !SavedFireProperties.bInitialized )
	{
		SavedFireProperties.AmmoClass = Class'SkaarjAmmo';
		SavedFireProperties.ProjectileClass = Class'DreadLAWProj';
	...
	Spawn(Class'DreadLAWProj',,,FireStart,FireRotation);
...
{
    local vector KFIncendiaryExplosionLoc;
    local KFNadeExplosion GibKFIncendiaryExplosion;
        ...
        HitDamageType != class'DamTypeBleedOut' )
    {
        ...
        {
			KFIncendiaryExplosionLoc = self.Location;
	        KFIncendiaryExplosionLoc.z += (CollisionHeight - (CollisionHeight * 0.5));

	        GibKFIncendiaryExplosion = Spawn(class 'KFIncendiaryExplosion',self,, KFIncendiaryExplosionLoc );
...
function BileBomb()
{
	BloatJet = spawn(class'BMTCustomMut.Dreadbilejet', self,,Location,Rotator(-PhysicsVolume.Gravity));
}
...
defaultproperties
{
     DetachedArmClass=Class'KFChar.SeveredLegClot'
     DetachedLegClass=Class'KFChar.SeveredLegClot'
     DetachedHeadClass=Class'KFChar.SeveredLegClot'
